// jspdf and jspdf-autotable are dynamically imported to reduce initial bundle size.
// They are only loaded when the user actually clicks "Print Invoice".

export const generateInvoicePDF = async (sale) => {
  // Dynamic imports — runs only when this function is called
  ...
  return new Promise((resolve) => {
    ...
    const buildPDF = (logoImg) => {
      ...
      // Add Paid and Due
      const safePaid = Number(sale.paidAmount) || 0;
      const safeDue = Number(sale.dueAmount) || 0;

      addText('Paid Amount :', sumX, finalY + 32, 9, 'normal', [22, 163, 74]); // Greenish
      addText(safePaid.toLocaleString(), sumValX, finalY + 32, 9, 'bold', [22, 163, 74], 'right');

      if (safeDue > 0) {
        addText('Due Amount :', sumX, finalY + 38, 9, 'normal', [220, 38, 38]); // Reddish
        addText(safeDue.toLocaleString(), sumValX, finalY + 38, 9, 'bold', [220, 38, 38], 'right');
      } else {
        addText('Status :', sumX, finalY + 38, 9, 'normal', [37, 99, 235]); // Blue
        addText('FULLY PAID', sumValX, finalY + 38, 9, 'bold', [37, 99, 235], 'right');
      }
