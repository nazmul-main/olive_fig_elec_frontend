...
export default function Sidebar({ isOpen, setIsOpen }) {
  ...
  const SidebarContent = () => (
    <div className="flex flex-col h-full bg-white border-r border-gray-200 shadow-xl md:shadow-none">
        ...
        <nav className="mt-5 flex-1 px-4 space-y-1">
          {filteredLinks.map((item) => {
            ...
            return (
              ...
              >
                <Icon className={`mr-3 h-5 w-5 flex-shrink-0 ${isActive ? 'text-white' : 'text-gray-400 group-hover:text-gray-500'}`} />
                {item.name}
              </Link>
            );
          })}
  ...
  return (
    <>
      ...
      <div className={`fixed inset-0 flex z-40 md:hidden transition-opacity duration-300 ${isOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
        ...
        {/* Sidebar panel */}
        <div className={`relative flex-1 flex flex-col max-w-xs w-full bg-white transition-transform duration-300 transform ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
